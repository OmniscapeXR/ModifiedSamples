 ...
 */
let accessKey = 'AVS+qhJbGLLK+J7/6EHTYlDCVVpyjloAYQN1lVyH7l4X';
/**
 ...
 */
let userId = 'PixelClient';
/**
 * The custom string to set as user data for your own peer (can be changed later).
 */
let userData = '';
// Used for controlling voice chat system
let Var_Media : OdinMedia;
/**
    ...
    try {
        ...
        navigator.mediaDevices
            ...
            .then(async(mediaStream) => {
             Var_Media = await odinRoom.createMedia(mediaStream);
             Var_Media.start().then(() => {
...
export class Application {
    ...
    registerCallbacks() {
        ...
        this.stream.addResponseEventListener("handle_responses", (response: string) => {
            ...
            switch (response) {
                case "ShowMouse":
                    console.log("Show mouse");
                    this.configUI.flagsUi.get(Flags.HoveringMouseMode).checkbox.checked = true;
                    this.configUI.flagsUi.get(Flags.HoveringMouseMode).setting.flag = true;   
                    this.stream.config.setFlagEnabled(Flags.HoveringMouseMode,true);
                    this.configUI.flagsUi.get(Flags.HoveringMouseMode).setting.updateURLParams();
                    break;
                case "HideMouse":
                    console.log("Hide mouse");
                    this.configUI.flagsUi.get(Flags.HoveringMouseMode).checkbox.checked = false;
                    this.configUI.flagsUi.get(Flags.HoveringMouseMode).setting.flag = false;
                    this.stream.config.setFlagEnabled(Flags.HoveringMouseMode,false);
                    this.configUI.flagsUi.get(Flags.HoveringMouseMode).setting.updateURLParams();
                    break;
                case "GetClientOS":
                    this.getClientOS();
            ...
            try{
                ...
                if(ParsedResponse && ParsedResponse.RoomID) //RoomID is a variable set in the project 
                {
                    roomId = ParsedResponse.RoomID; // we set the plugins roomId to our RoomID from unreal
                    console.log(roomId);
    ...
    getClientOS(){
        ...
        if (window.navigator.userAgent.indexOf("Windows") != -1) {
            this.stream.emitUIInteraction("Windows");
        } else if (window.navigator.userAgent.indexOf("Android") != -1) {
            this.stream.emitUIInteraction("Android");
        } else if (window.navigator.userAgent.indexOf("iPhone") != -1) {
            this.stream.emitUIInteraction("iPhone");
        } else if (window.navigator.userAgent.indexOf("Mac OS") != -1) {
            this.stream.emitUIInteraction("MacOS");
        } else if (window.navigator.userAgent.indexOf("Linux") != -1) {
            this.stream.emitUIInteraction("Linux");
        } else {
    ...
    getApartmentID(){
        ...
        if (apartmentID)
        {
            this.stream.emitUIInteraction("ApartmentID");
        }
    }
}